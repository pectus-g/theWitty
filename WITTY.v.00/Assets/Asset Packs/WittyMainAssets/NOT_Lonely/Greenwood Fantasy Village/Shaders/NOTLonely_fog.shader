...
Shader "NOT_Lonely/NOTLonely_Fog"
{
	Properties
	{
		...
		[ASEEnd]_NoiseOffset("Noise Offset", Vector) = (0,0,0,0)
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float4 ShadowCoords = float4( 0, 0, 0, 0 );
					...
					#endif
				#endif
				float4 _Color_Instance = UNITY_ACCESS_INSTANCED_PROP(NOT_LonelyNOTLonely_Fog,_Color);
				
				float2 uv_Mask5 = IN.ase_texcoord3.xy;
				float mulTime56 = _TimeParameters.x * -0.02;
				float2 appendResult26 = (float2(_AnimSpeed , 0.0));
				float2 _NoiseOffset_Instance = UNITY_ACCESS_INSTANCED_PROP(NOT_LonelyNOTLonely_Fog,_NoiseOffset);
				float2 texCoord29 = IN.ase_texcoord3.xy * _NoiseTiling + _NoiseOffset_Instance;
				float2 panner24 = ( mulTime56 * appendResult26 + texCoord29);
				float4 tex2DNode7 = SAMPLE_TEXTURE2D( _Noise, sampler_Noise, panner24 );
				...
				float3 Color = ( IN.ase_color * _Color_Instance ).rgb;
				float Alpha = saturate( ( IN.ase_color.a * ( ( SAMPLE_TEXTURE2D( _Mask, sampler_Mask, uv_Mask5 ).r * pow( abs( ( clampResult104 + tex2DNode7.r ) ) , _NoiseContrast ) ) * _Color_Instance.a * clampResult42 * clampResult45 * ( 1.0 - clampResult49 ) ) ) );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float2 uv_Mask5 = IN.ase_texcoord2.xy;
				float mulTime56 = _TimeParameters.x * -0.02;
				float2 appendResult26 = (float2(_AnimSpeed , 0.0));
				float2 _NoiseOffset_Instance = UNITY_ACCESS_INSTANCED_PROP(NOT_LonelyNOTLonely_Fog,_NoiseOffset);
				float2 texCoord29 = IN.ase_texcoord2.xy * _NoiseTiling + _NoiseOffset_Instance;
				float2 panner24 = ( mulTime56 * appendResult26 + texCoord29);
				float4 tex2DNode7 = SAMPLE_TEXTURE2D( _Noise, sampler_Noise, panner24 );
				float clampResult104 = clamp( tex2DNode7.r , 0.0 , 1.0 );
				float4 _Color_Instance = UNITY_ACCESS_INSTANCED_PROP(NOT_LonelyNOTLonely_Fog,_Color);
...
/*ASEBEGIN
Version=18933
0;73;2560;1286;978.1607;694.658;1.3;True;False
...
WireConnection;96;2;108;0
WireConnection;96;3;110;0
ASEEND*/
//CHKSM=A268178D80CC080F537802B0A8E7BA859D5F6DA2