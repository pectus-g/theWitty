namespace NOT_Lonely
{
    ...
    public class NL_LookOrbit : MonoBehaviour
    {

        public Transform target;
        private Transform followTarget;
        private Transform dummy;
        private NL_RollerBall rollerBall;
        ...
        void Start()
        {
            ...
            dummy.parent = followTarget;
            dummy.localPosition = Vector3.zero;
            dummy.localEulerAngles = Vector3.zero;

            transform.parent = dummy;
            transform.localPosition = Vector3.zero;
            transform.localEulerAngles = Vector3.zero;

            Vector3 angles = dummy.eulerAngles;
            x = angles.y;
            y = angles.x;
            ...
            rollerBall = target.GetComponent<NL_RollerBall>();
        ...
        void Update()
        {
            if (target && !CamFreeze && !RotationFreeze)
            {
                ...
                dummy.position = Vector3.Slerp(dummy.position, position, 10 * Time.deltaTime);
                dummy.rotation = rotation;

                if(rollerBall != null) rollerBall.forwardDirection = dummy.forward;
            }
            dummy.LookAt(followTarget);
        }
